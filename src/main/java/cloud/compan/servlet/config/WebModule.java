...
public class WebModule extends AbstractModule {
    ...
    protected void configure() {
        if (Boolean.getBoolean("debug.components")) System.out.println(" 配置WebModule...");
        
        // 1. 绑定路由注册器 - 单例模式，线程安全
        bind(RouteRegistry.class).in(Singleton.class);
        if (Boolean.getBoolean("debug.components")) System.out.println("RouteRegistry 已绑定为单例");
        
        // 2. 绑定请求分发器 - 单例模式，处理所有HTTP请求
        bind(RequestDispatcher.class).in(Singleton.class);
        if (Boolean.getBoolean("debug.components")) System.out.println("RequestDispatcher 已绑定为单例");
        
        // 3. 绑定控制器扫描器 - 单例模式，启动时扫描控制器
        bind(ControllerScanner.class).in(Singleton.class);
        if (Boolean.getBoolean("debug.components")) System.out.println("ControllerScanner 已绑定为单例");
        
        if (Boolean.getBoolean("debug.components")) System.out.println("WebModule 配置完成");
    }
} 