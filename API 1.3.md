...
| 字段名     | 类型      | 说明             |
| ------- | ------- | -------------- |
| code    | Integer | HTTP 状态码（业务语义） |
| message | String  | 简要的响应描述        |
| data    | Object  | 实际返回数据（结构各异）   |
|         |         |                |
## 通用响应格式
...
### 4. 访问分享内容
```http
POST /api/v1/shares/{share_id}/access
Content-Type: application/json
...
### 2. 恢复文件/文件夹
```http
POST /api/v1/recycle-bin/{item_id}/restore?item_type=file
Authorization: Bearer <token>
Content-Type: application/json
...
### 3. 彻底删除
```http
DELETE /api/v1/recycle-bin/{item_id}?item_type=folder
Authorization: Bearer <token>
```
响应格式：
```json
